...
def compute_PCA(task_matrix, masker, output_name):
    ...
    if task_matrix.shape[1] == 45:
        ...
        task_list = GROUP_LIST.remove('Rest')
    else:
        task_list = GROUP_LIST
    print(GROUP_LIST)
    # set pca to explain 95% of variance
    pca = PCA(.95)
    PCA_components = pca.fit_transform(np.mean(task_matrix, axis=2))
...
def greene_method(task_matrix, task_list=GROUP_LIST, threshold=0.99):
    ...
    df = pd.DataFrame(index=range(VOXELS), columns=['Task Name', 'Max Task value', 'Is Specific'])
